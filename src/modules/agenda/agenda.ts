...
export async function schedule<T extends Tasks>(
  when: Date | string,
  name: T,
  data: TaskOptions<T>,
): Promise<Job<TaskOptions<T>>> {
...
export async function every<T extends Tasks>(
  interval: string,
  name: T,
  data: TaskOptions<T>,
): Promise<Job<TaskOptions<T>>> {
...
export async function cancel<T extends Tasks>(
  name: T,
  params: Partial<IJobParameters<Partial<TaskOptions<T>>>>,
): Promise<number> {
  const flatten = Object.entries(params.data).reduce(
...
export async function jobs<T extends Tasks>(
  name: T,
  params: Partial<IJobParameters<Partial<TaskOptions<T>>>>,
): Promise<Job<TaskOptions<T>>[]> {
